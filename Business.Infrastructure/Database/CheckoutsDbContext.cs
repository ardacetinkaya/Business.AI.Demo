...
public class CheckoutsDbContext(DbContextOptions<CheckoutsDbContext> options) : DbContext(options), IOrdersPayment
{
    ...
    public async Task<(Order order, Payment payment)> ExecuteInTransactionAsync(Func<CancellationToken, Task<(Order order, Payment payment)>> action, CancellationToken cancellationToken = default)
    {
